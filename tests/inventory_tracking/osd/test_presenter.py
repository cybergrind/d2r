...
def test_health_threshold_includes_exactly_seventy_percent(snapshot, render):
    ...
    assert render(replace(state, current_raw=700 * 256 + 1), now=101) == []
    assert render(replace(state, current_raw=699 * 256), now=101) == ['699/1000']
...
def test_incomplete_read_and_startup_are_quiet(render):
    assert render(State(100, reason='incomplete read'), now=101) == []
    assert render(State(100, reason='connecting'), now=101) == []
...
def test_refill_clears_previous_shortage(snapshot, render):
    ...
    assert render(replace(state, belt_contents=(531, 531, 606, 606) * 4), now=101) == []
...
def test_explicit_targets_count_all_potion_tiers(snapshot):
    ...
    presenter = Presenter(replace(OSD, rejuvenation_target=5, healing_target=7))
