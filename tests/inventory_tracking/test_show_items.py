...
from inventory_tracking.layout import SHOW_ITEMS_RVA
from inventory_tracking.models import ObservationStatus
from inventory_tracking.show_items import observe_show_items
...
def test_label_observation_rejects_partial_invalid_and_changing_reads(reads, expected):
    with (
        patch('inventory_tracking.show_items.identity', return_value=TOKEN),
        patch('inventory_tracking.show_items.process_mappings', return_value=[MAPPING]),
        patch('inventory_tracking.show_items.os.open', return_value=9),
        patch('inventory_tracking.show_items.os.close') as close,
        patch('inventory_tracking.show_items.os.pread', side_effect=reads) as read,
    ):
...
def test_label_observation_suppresses_process_mapping_and_access_failures(change):
    with (
        patch(
            'inventory_tracking.show_items.identity',
            side_effect=[TOKEN, {}] if change == 'identity' else [TOKEN, TOKEN],
        ),
        patch(
            'inventory_tracking.show_items.process_mappings',
            side_effect=[[MAPPING], []] if change == 'mapping' else [[MAPPING], [MAPPING]],
        ),
        patch('inventory_tracking.show_items.os.open', return_value=9),
        patch('inventory_tracking.show_items.os.close'),
        patch(
            'inventory_tracking.show_items.os.pread',
