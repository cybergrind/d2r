...
from inventory_tracking.tracking.consume import observe_consume, read_consume


CASES = json.loads((Path(__file__).parents[1] / 'fixtures/consume_effects.json').read_text())
...
def test_process_change_refuses_memory_access(monkeypatch):
    monkeypatch.setattr('inventory_tracking.tracking.consume.process_mappings', lambda _: [])
    monkeypatch.setattr('inventory_tracking.tracking.consume.identity', lambda _: {'pid': 2})

    def forbidden(*args):
        pytest.fail('Changed process must not be opened')

    monkeypatch.setattr('inventory_tracking.tracking.consume.os.open', forbidden)
