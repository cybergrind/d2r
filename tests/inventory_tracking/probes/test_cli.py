...
from inventory_tracking.native.process import executable_name
from inventory_tracking.native.session import inspect_game
...
def test_unnamed_readable_mapping_still_probes_memory():
    ...
    with (
        patch('inventory_tracking.native.session.process_info', return_value={'identity': token}),
        patch('inventory_tracking.native.process.Path.read_text', return_value='1000-2000 rw-p 0 00:00 0'),
        patch('inventory_tracking.native.session.identity', return_value=token),
        patch('inventory_tracking.native.session.fingerprint_executable', return_value={'error': 'not found'}),
        patch('inventory_tracking.native.session.vm_read', return_value={'ok': True}) as read,
        patch('inventory_tracking.native.session.proc_read', return_value={'ok': True}),
    ):
...
def test_restart_during_fingerprinting_invalidates_successful_read():
    ...
    with tempfile.TemporaryDirectory() as directory:
        ...
        with (
            patch('inventory_tracking.native.session.process_info', return_value={'identity': before}),
            patch('inventory_tracking.native.session.process_mappings', return_value=[mapping]),
            patch('inventory_tracking.native.session.identity', side_effect=[before, before, after]),
            patch('inventory_tracking.native.session.vm_read', return_value={'ok': True}),
            patch('inventory_tracking.native.session.proc_read', return_value={'ok': True}),
        ):
            result = inspect_game(os.getpid())
