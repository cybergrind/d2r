...
from inventory_tracking.probes.buff import collect_candidates, collect_effect_lists, record
...
def test_interrupted_recording_publishes_completion(monkeypatch, tmp_path):
    monkeypatch.setattr('inventory_tracking.probes.buff.LiveReader.connect', lambda *_: (1, {'identity': {}}, {}))
    monkeypatch.setattr('inventory_tracking.probes.buff.capture_sample', lambda *_: {'stable': True})

    def stop(_):
        raise KeyboardInterrupt

    monkeypatch.setattr('inventory_tracking.probes.buff.time.sleep', stop)
