...
from inventory_tracking.images import discover_images, read_pe
...
def test_restart_during_scan_discards_candidates():
    ...
    from inventory_tracking.image_probe import inspect_images

    token = {'pid': 123, 'start_ticks': '1'}
    game = {'identity': token, 'executable_fingerprint': {'error': 'missing'}}
    with (
        patch('inventory_tracking.image_probe.identity', side_effect=[token, token, {'pid': 123, 'start_ticks': '2'}]),
...
def test_unrelated_mapping_changes_do_not_discard_image():
    ...
    with (
        ...
        patch('inventory_tracking.image_probe.os.open', return_value=77),
        patch('inventory_tracking.image_probe.os.close'),
        patch('inventory_tracking.image_probe.os.pread', side_effect=lambda fd, a, o: data[o - 8192 : o - 8192 + a]),
    ):
        result = inspect_images(123, game)
    assert result['status'] == 'candidate'
