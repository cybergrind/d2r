...
from inventory_tracking.unit_probe import inspect_units
from inventory_tracking.units import walk_units
...
def snapshot(*, mutation=None, late=False):
    ...
    with tempfile.TemporaryDirectory() as directory, ExitStack() as stack:
        ...
        def mock(name, **kwargs):
            return stack.enter_context(patch('inventory_tracking.unit_probe.' + name, **kwargs))
...
def test_indexed_reader_keeps_gap_and_permission_checks():
    from inventory_tracking.unit_probe import ResearchReader
    ...
    with patch('inventory_tracking.unit_probe.os.pread', return_value=b'abcdef') as read:
