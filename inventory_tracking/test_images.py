...
from .images import discover_images, read_pe
...
class ImageTests(unittest.TestCase):
    def test_relocated_anonymous_image_matches_disk_headers(self):
        data = fixture()
        disk = read_pe(lambda offset, size: data[offset : offset + size])
        base = 0x200000000
        result = discover_images(
            lambda address, size: data[address - base : address - base + size], [mapping(base)], disk
        )
        assert result['status'] == 'candidate'
        assert result['candidate_base'] == base
        assert result['images'][0]['pe']['sections'][0]['rva'] == 4096

    def test_duplicate_header_matches_are_ambiguous(self):
        data = fixture()
        disk = read_pe(lambda offset, size: data[offset : offset + size])
        result = discover_images(
    ...
    def test_non_executable_header_copy_is_not_a_loaded_image(self):
        data = fixture()
        disk = read_pe(lambda offset, size: data[offset : offset + size])
        copied = mapping(8192)
    ...
    def test_fragmented_code_with_executable_entry_is_a_candidate(self):
        data = fixture()
        disk = read_pe(lambda offset, size: data[offset : offset + size])
        header = mapping(8192)
    ...
    def test_short_reads_and_unbounded_headers_are_rejected(self):
    ...
    def test_mapping_gap_prevents_cross_mapping_header_read(self):
        calls = []
        data = fixture()
        disk = read_pe(lambda offset, size: data[offset : offset + size])
        entry = mapping(8192)
    ...
    def test_read_budget_stops_scan_without_claiming_unique_match(self):
        data = fixture()
        disk = read_pe(lambda offset, size: data[offset : offset + size])
        result = discover_images(
            lambda address, size: data[address % 8192 : address % 8192 + size],
...
class ImageWorkflowTests(unittest.TestCase):
    def test_restart_during_scan_discards_candidates(self):
        ...
        with (
            patch(
                'inventory_tracking.image_probe.identity', side_effect=[token, token, {'pid': 123, 'start_ticks': '2'}]
            ),
            patch('inventory_tracking.image_probe.process_mappings', return_value=[]),
            patch('inventory_tracking.image_probe.os.open', return_value=77),
            patch('inventory_tracking.image_probe.os.close'),
            patch('inventory_tracking.image_probe.discover_images', return_value={'candidate_base': 4096}),
        ):
            result = inspect_images(123, game)
        assert result['status'] == 'stale'
        assert 'candidate_base' not in result

    def test_unrelated_mapping_changes_do_not_discard_image(self):
        from unittest.mock import patch
        ...
        with (
            patch('inventory_tracking.image_probe.identity', return_value=token),
            patch('inventory_tracking.image_probe.disk_headers', return_value=({}, disk)),
            patch('inventory_tracking.image_probe.process_mappings', side_effect=[before, after]),
            patch('inventory_tracking.image_probe.os.open', return_value=77),
            patch('inventory_tracking.image_probe.os.close'),
            patch(
                'inventory_tracking.image_probe.os.pread', side_effect=lambda fd, a, o: data[o - 8192 : o - 8192 + a]
            ),
        ):
            result = inspect_images(123, game)
        assert result['status'] == 'candidate'

    def test_read_mapping_change_discards_image(self):
        from unittest.mock import patch
        ...
        with (
            patch('inventory_tracking.image_probe.identity', return_value=token),
            patch('inventory_tracking.image_probe.disk_headers', return_value=({}, disk)),
            patch('inventory_tracking.image_probe.process_mappings', side_effect=[[mapping(8192)], []]),
            patch('inventory_tracking.image_probe.os.open', return_value=77),
            patch('inventory_tracking.image_probe.os.close'),
            patch(
                'inventory_tracking.image_probe.os.pread', side_effect=lambda fd, a, o: data[o - 8192 : o - 8192 + a]
