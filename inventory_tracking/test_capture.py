...
from .capture import capture_ranges, scan_unit_table


class CaptureTests(unittest.TestCase):
    def test_short_reads_preserve_holes_and_boundaries(self):
        calls = []
    ...
    def test_budget_rejected_before_any_read(self):
        with pytest.raises(ValueError, match='Capture exceeds byte budget'):
            capture_ranges(lambda *args: self.fail('must not read'), [(0, 100)], io.BytesIO(), max_bytes=99)

    def test_signature_spans_contiguous_blocks_but_not_holes(self):
    ...
    def test_restart_after_capture_suppresses_signature_results(self):
        import tempfile
