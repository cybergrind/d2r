...
from .linux_process import executable_name
from .memory import proc_read, vm_read
from .probe import inspect_game
from .reports import publish


class DiagnosticsTests(unittest.TestCase):
    def test_unnamed_readable_mapping_still_probes_memory(self):
    ...
    def test_restart_during_fingerprinting_invalidates_successful_read(self):
    ...
    def test_windows_process_names_exclude_launchers(self):
    ...
    def check_child_read(self, reader):
        ...
        try:
            ...
            if result.get('errno') in (1, 13):
                self.skipTest(f'{reader.__name__} denied by execution environment: {result}')
    ...
    def test_proc_mem_reads_known_child_memory(self):
        self.check_child_read(proc_read)

    def test_process_vm_reads_known_child_memory(self):
        self.check_child_read(vm_read)

    def test_failed_probe_is_logged_and_watchable(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            watch = subprocess.run(
                [*base, 'watch', '--include-existing', '--timeout', '2'], capture_output=True, text=True
            )
    ...
    def test_watcher_detects_running_then_complete(self):
        with tempfile.TemporaryDirectory() as directory:
