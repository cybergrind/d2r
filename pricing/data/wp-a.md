...
## Variants pass (2026-09-20)

The 2026-09-18 extraction read only the gear table and the merc table. Every guide also has variant tabs (Starter / Standard / Magic Find / Ubers / Set Build / Hardcore / …) whose loadouts live in prose plus an embedded d2planner profile. Five sub-agents re-read all 26 guides (cached HTML + live page + planner) into `wp-a-variants/<slug>.json` (schema: `wp-a-variants/SCHEMA.md`); `tools/wp_a_variants.py --write` merges them into `wp-a-builds.json` under `variants` / `prose_only_items` / `variants_sources` and writes `wp-a-variants/index.json` (item → build/variant/side) and `wp-a-variants/new-demand.md` (items absent from the main tables, weighted S=2/A=1).

Planner method: the guide page's `data-d2-id` → `https://planners.maxroll.gg/profiles/d2/<id>` (all profiles: equipped / swap / cube / inventory / merc item codes); names via `https://assets-ng.maxroll.gg/d2planner/game/data.json` + `strings.json` (pre-RotW ids) and the blizzhackers d2data dump for RotW ids (runeword id = RunewordNN row + 27). Hireling ids seen: 1 Rogue Cold, 6 Act 2 Prayer, 10 Holy Freeze, 11 Might, 28 Act 5 Bash, 36/38 Act 5 Frenzy. Planner "Crafted <Sunder>" = maxroll "Renewed <Sunder>". Known caveats: several guides' prose is stale against their planner (recorded per file under `discrepancies` / `notes`); d2data spellings were normalised (War Traveler, Vampire Gaze, Thundergod's, Verdungo's, Gore Rider).

Headline misses fixed: Sazabi's Grand Tribute on the Echoing Strike uber merc; Act 5 Frenzy mercs with 2× Plague Phase Blade on the Lightning Fury / Lightning Sorc / Lightning Strike uber variants; Tal Rasha's full set in the Blizzard and Meteor "Set Build" tabs; Angelic Wings + Halo and Sigon's pieces as starter kits in five builds; Hellfire Torch, Gheed's and the colossal jewels in every endgame planner but missing from the tables.
